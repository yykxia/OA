...
namespace WX2HK
{
    public partial class selectUser : System.Web.UI.Page
    {
        ...
        {
            Grid2.DataSource = getDeptUserList("3");
            Grid2.DataBind();
        }
        ...
        //获取成员列表
        public static DataTable getDeptUserList(string deptId)
        {
            try
            {
                DataTable dt = new DataTable();
                string accessToken = VerifyLegal.GetAccess_Token();
                //status=0：状态为所有人员；fetch_child=1：递归获取所有子集部门人员
                string wxUrl = string.Format("https://qyapi.weixin.qq.com/cgi-bin/user/list?access_token={0}&department_id={1}&fetch_child=1&status=0", accessToken,deptId);
                var client = new System.Net.WebClient();
                client.Encoding = System.Text.Encoding.UTF8;
                var data = client.DownloadString(wxUrl);
                //DataTable userList = JsonToDataTable(data);
                ...
                {
                    Alert.Show(string.Format("getDeptUserList:{0}", returnObj.errmsg));
                    return null;
                }
            }
            catch (Exception ex)
