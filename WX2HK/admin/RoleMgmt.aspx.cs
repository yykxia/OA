    ...
    public partial class RoleMgmt : System.Web.UI.Page
    {
        ...
        protected void bindGrid()
        {
            try
            {
                ...
                string sqlCmd = "select * from OA_Sys_Role order by id";
                SqlSel.GetSqlSel(ref dt, sqlCmd);
                Grid1.DataSource = dt;
                Grid1.DataBind();

                sqlCmd = "select max(id) from OA_Sys_Role";
        ...
        protected void deleteFromDb(int DbId)
        {
            if (DbId != 1)//非管理员用户
            {
                string sqlCmd = "delete from OA_Sys_Role where id=" + DbId;
                SqlSel.ExeSql(sqlCmd);
                bindGrid();
            }
            else 
            {
                Alert.ShowInTop("管理员帐号不可删除！");
                return;
            }
        }
