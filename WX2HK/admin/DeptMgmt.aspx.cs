...
{
    public partial class DeptMgmt : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                defaultObj.Add("deptName", "");
                defaultObj.Add("deptDesc", "");
        ...
        protected void bindGrid()
        {
            try
            {
                ...
                string sqlCmd = "select * from OA_sys_department order by id";
                SqlSel.GetSqlSel(ref dt, sqlCmd);
                Grid1.DataSource = dt;
                Grid1.DataBind();

                sqlCmd = "select max(id) from OA_sys_department";
        ...
        protected void btn_save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (int rowIndex in modifiedDict.Keys)
                {
                    ...
                    sqlCmd = "update OA_sys_department set [deptName]='" + Grid1.Rows[rowIndex].Values[0].ToString() + "',";
                    sqlCmd += "deptDesc='" + Grid1.Rows[rowIndex].Values[1].ToString() + "' ";
                ...
                for (int i = 0; i < newAddedList.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(newAddedList[i]["deptName"].ToString()))
                    {
                        Alert.ShowInTop("名称不可为空！");
                        return;
                    }
                    sqlCmd = "insert into OA_sys_department (deptName,deptDesc) values ";
                    sqlCmd += "('" + newAddedList[i]["deptName"].ToString().Trim() + "','" + newAddedList[i]["deptDesc"].ToString() + "')";
                    SqlSel.ExeSql(sqlCmd);
        ...
        {
            string sqlCmd = "delete from OA_sys_department where id=" + DbId;
            SqlSel.ExeSql(sqlCmd);
            bindGrid();
        }
