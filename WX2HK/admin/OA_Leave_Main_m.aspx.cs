...
{
    public partial class OA_Leave_Main_m : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                string curPageName = string.Format("admin%2fOA_Leave_Main_m.aspx?tabId={0}", empId);
                ...
                string curUrl = string.Format("admin/OA_Leave_Main_m.aspx?tabId={0}&code={1}&state=STATE", empId, code);
        ...
        {
            string sqlCmd = "select A.*,B.stepId,c.chineseName,d.deptName,t.flowName from OA_Leave_Main A left join OA_Sys_step_empList B on B.formId=A.id";
            sqlCmd += " left join OA_Sys_EmployeeInfo c on c.id=a.reqMan left join OA_sys_department d on d.id=c.deptId";
            sqlCmd += " left join OA_sys_flow t on t.id=A.flowId where B.id='" + empId + "'";
            ...
            //加载附件信息
            bindHidden(formId, "OA_Leave_Main");
            //加载证明人信息
            //loadProvInfo(formId);
            //审批信息
            bindCheckDetail(formId, "OA_Leave_Main");
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (curStepId == stepId)
            {
                sqlCmd = "select * from OA_Sys_Flow_Step where flowId=(select flowId from OA_Leave_Main where id ='" + formId + "')";
                ...
                if (dr.Length > 0)
                {
                    ...
                    sqlCmd = "update OA_Leave_Main set CurrentStepID='" + nextStepId + "' where id='" + formId + "'";
                    SqlSel.ExeSql(sqlCmd);

                    //执行消息推送和审批人员的添加
                    pushMessage(nextStepId, "OA_Leave_Main", formId, "考勤申请");
                }
                else
                {
                    //流程结束
                    sqlCmd = "update OA_Leave_Main set CurrentStepID='0' where id='" + formId + "'";
                    SqlSel.ExeSql(sqlCmd);
                    //只会流程发起人
                    WX2HK.ReturnInfo.pushMessage(txb_Hidden_reqMan.Text, "审批完成", "", "您提交的考勤申请已审批完成。", "");
                }
