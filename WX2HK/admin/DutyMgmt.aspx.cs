...
{
    public partial class DutyMgmt : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                defaultObj.Add("DutyName", "");
                defaultObj.Add("DutyDesc", "");
        ...
        protected void bindGrid()
        {
            try
            {
                ...
                string sqlCmd = "select * from OA_sys_Duties order by id";
                SqlSel.GetSqlSel(ref dt, sqlCmd);
                Grid1.DataSource = dt;
                Grid1.DataBind();

                sqlCmd = "select max(id) from OA_sys_Duties";
        ...
        protected void btn_save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (int rowIndex in modifiedDict.Keys)
                {
                    ...
                    sqlCmd = "update OA_sys_Duties set [DutyName]='" + Grid1.Rows[rowIndex].Values[0].ToString() + "',";
                    sqlCmd += "DutyDesc='" + Grid1.Rows[rowIndex].Values[1].ToString() + "' ";
                ...
                for (int i = 0; i < newAddedList.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(newAddedList[i]["DutyName"].ToString()))
                    {
                        Alert.ShowInTop("名称不可为空！");
                        return;
                    }
                    sqlCmd = "insert into OA_sys_Duties (DutyName,DutyDesc) values ";
                    sqlCmd += "('" + newAddedList[i]["DutyName"].ToString().Trim() + "','" + newAddedList[i]["DutyDesc"].ToString() + "')";
                    SqlSel.ExeSql(sqlCmd);
