...
{
    public partial class OA_Bills_Main : BasePage
    {
        ...
        private void loadInfo(string empId)
        {
            ...
            sqlCmd += " left join OA_Sys_EmployeeInfo c on c.id=a.reqMan left join OA_sys_department d on d.id=c.deptId left join OA_sys_Project t on t.id=a.RelProj where B.id='" + empId + "'";
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (curStepId == stepId)
            {
                sqlCmd = "select * from OA_Sys_Flow_Step where flowId=(select flowId from OA_Bills_Main where id ='" + formId + "')";
                ...
                if (dr.Length > 0)
                {
                    ...
                    sqlCmd = "update OA_Bills_Main set CurrentStepID='" + nextStepId + "' where id='" + formId + "'";
                    SqlSel.ExeSql(sqlCmd);

                    //执行消息推送和审批人员的添加
                    pushMessage(nextStepId, "OA_Bills_Main", formId, "费用报销");
                }
                else
                {
                    //流程结束
                    sqlCmd = "update OA_Bills_Main set CurrentStepID='0' where id='" + formId + "'";
                    SqlSel.ExeSql(sqlCmd);
                    //只会流程发起人
                    WX2HK.ReturnInfo.pushMessage(txb_Hidden_reqMan.Text, "审批完成", "", "您提交的报销申请已审批完成。", "");
                }
