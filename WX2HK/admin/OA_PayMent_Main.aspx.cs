...
{
    public partial class OA_PayMent_Main : BasePage
    {
        ...
        private void loadInfo(string empId)
        {
            ...
            label_payeeName.Text = dt.Rows[0]["payeeName"].ToString();
            label_total.Text = dt.Rows[0]["payTotal"].ToString();
            label_reason.Text = dt.Rows[0]["others"].ToString();
            label_proj.Text = dt.Rows[0]["projName"].ToString();
            label_isAdvPay.Text = dt.Rows[0]["payType"].ToString();
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (curStepId == stepId)
            {
                sqlCmd = "select * from OA_Sys_Flow_Step where flowId=(select flowId from OA_PayMent_Main where id ='" + formId + "')";
                ...
                if (dr.Length > 0)
                {
                    ...
                    sqlCmd = "update OA_PayMent_Main set CurrentStepID='" + nextStepId + "' where id='" + formId + "'";
                    SqlSel.ExeSql(sqlCmd);

                    //执行消息推送和审批人员的添加
                    pushMessage(nextStepId, "OA_PayMent_Main", formId, "付款申请");
                }
                else
                {
                    sqlCmd = "update OA_PayMent_Main set CurrentStepID='0' where id='" + formId + "'";
                    SqlSel.ExeSql(sqlCmd);

                    //只会流程发起人
                    WX2HK.ReturnInfo.pushMessage(txb_Hidden_reqMan.Text, "审批完成", "", "您提交的付款申请已审批完成。", "");
                }
